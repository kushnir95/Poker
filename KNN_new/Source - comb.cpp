...
vector<Mat> prepare_parts(Mat image) {
	Mat part = image(Rect(0, 0, image.cols / 5, image.rows / 3));

	Mat part_rank1 = part(Rect(0, 0, part.cols, part.rows / 2));
	Mat part_suit1 = part(Rect(0, part.rows / 2, part.cols, part.rows / 2));

	Mat part_rank, part_suit;
	
	Mat temp_part;
	for (int y = 10; y < image.rows / 10; y += 10) {
		temp_part = part(Rect(0, y, part.cols, part.rows / 2));
		if (contourKids(temp_part)[0] > contourKids(part_rank1)[0]){
			part_rank = temp_part.clone();
		}
	}
	if (!part_rank.size().area()) part_rank = part_rank1.clone();
			
	temp_part = part(Rect(0, part.rows / 2 - 10, part.cols, part.rows / 2));
	if (contourKids(temp_part)[0] > contourKids(part_suit1)[0]) {
		part_suit = temp_part.clone();
	}
	temp_part = part(Rect(0, part.rows / 2 + 10, part.cols, part.rows / 2));
	if (contourKids(temp_part)[0] > contourKids(part_suit1)[0]) {
		part_suit = temp_part.clone();
	}
	if (!part_suit.size().area()) part_suit = part_suit1.clone();

	return { part_suit, part_rank };
}
...
Card matchCart(Mat image, int idx) {
	...
	vector<Mat> parts = prepare_parts(image);
	Mat part_suit = parts[0];
	Mat part_rank = parts[1];
	...
	for (Mat m : suits) {
		suit_desc.push_back(Features_Mapper::prepare_descriptors(m));
	}
	Mat descriptors;
	vconcat(suit_desc, descriptors);

	Mat predict;
	SuitKNN->findNearest(descriptors, 60, predict);
	card_suit = findFrequentlyClass(predict);

	
	Vec2i rank_kids = contourKids(part_rank);

	vector<Mat> ranks = generate(part_rank, 5);
	vector<Mat> rank_desc;
	for (Mat m : ranks) {
		rank_desc.push_back(Features_Mapper::prepare_descriptors(m, 16));
	}
	descriptors;
	vconcat(rank_desc, descriptors);

	if (rank_kids[1] == 2) {
		EightKNN->findNearest(descriptors, 20, predict);
	}
	else if (rank_kids[1] == 1) {
		SixKNN->findNearest(descriptors, 60, predict);
	}else TwoKNN->findNearest(descriptors, 60, predict);
