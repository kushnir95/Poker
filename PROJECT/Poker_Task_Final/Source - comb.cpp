...
int match(Mat& image, const Ptr<cv::ml::KNearest>& Two, const Ptr<cv::ml::KNearest>& One, const Ptr<cv::ml::KNearest>& Zero) {
	Mat rank_desc = Features_Mapper::prepare_descriptors(image, 1);
	...
	return (int)predict.at<float>(0,0);
}
...
Card matchCart(Mat image, int idx) {
	...
	Mat suit_desc = Features_Mapper::prepare_descriptors(part_suit, 0);
	Mat predict;
	SuitKNN->findNearest(suit_desc, 60, predict);
	card_suit = (int)predict.at<float>(0,0);
	...
	/*imwrite("./predict/" + to_string(idx) + "_suit_" + imgName, part_suit);
	imwrite("./predict/" + to_string(idx) + "_rank_" + imgName, part_rank);*/

	Mat rank_desc = Features_Mapper::prepare_descriptors(part_rank, 1);
	if (rank_kids == 2) {
	...
	card_rank = (int)predict.at<float>(0, 0);
